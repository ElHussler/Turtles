...
class LaserRoomba():
    ...
    def __init__(self, name):
        ...
        self.laser_sub = rospy.Subscriber(  # Creating a subscriber listening to the laser scans
            "/turtlebot_1/scan",                          # The topic to which it should listend
    ...
    def laser_callback(self, msg):
        ...
        if min_distance < 1:                    # If the robot is close, then
            ...
            end_time = now + randint(1,3)                     # Defining for how long the robot should turn.
            angle_velocity = np.pi/2                     # Defining the angular velocity and direction of the turn. Use np.pi to get PI.
            while now < end_time:                               # While the run time is not up, do
                ...
                rate.sleep()
                now = rospy.Time.now().to_sec()     # Sleep to ensure rate of 10hz
        else:                                    # If the robot is far away from everything, then
            rospy.loginfo("Straight")
            twist_msg.linear.x = 0.6                # Set linear movement
